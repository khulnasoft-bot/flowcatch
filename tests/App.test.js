...
describe('<App />', () => {
  ...
  describe('render', () => {
    ...
    it('respects the filter', () => {
      wrapper.setState({ log: [issuesOpened, issuesOpenedTwo], filter: 'repository.name:khulnasoft-bot' })
      expect(wrapper.find('ListItem').length).toBe(1)
    })

    it('respects the filter if it starts with body', () => {
      wrapper.setState({ log: [issuesOpened, issuesOpenedTwo], filter: 'body.repository.name:khulnasoft-bot' })
      expect(wrapper.find('ListItem').length).toBe(1)
    })
  ...
  describe('onmessage', () => {
    it('adds the new log to the state and localStorage', () => {
      ...
      expect(localStorage.setItem.mock.calls[0][0]).toBe('FLOWCATCH:log:CHANNEL')
  ...
  describe('togglePinned', () => {
    ...
    it('stores the pinnedDeliveries in localStorage', () => {
      wrapper.instance().togglePinned(123)
      expect(localStorage.setItem.mock.calls[0][0]).toBe('FLOWCATCH:log:CHANNEL:pinned')
      expect(localStorage.setItem.mock.calls[0][1]).toMatchSnapshot()

      wrapper.instance().togglePinned(123)
      expect(localStorage.setItem.mock.calls[1][0]).toBe('FLOWCATCH:log:CHANNEL:pinned')
